...
#include "hal_data.h"
#include "C:\Users\Sejeong\ra6m3_ws\Renesas_SummerCamp_CAN_Ex2\AppWizard\Source\CustomCode\Application.h"
#include "C:\Users\Sejeong\ra6m3_ws\Renesas_SummerCamp_CAN_Ex2\AppWizard\Source\Generated\Resource.h"
#include "C:\Users\Sejeong\ra6m3_ws\Renesas_SummerCamp_CAN_Ex2\AppWizard\Source\Generated\ID_SCREEN_00.h"
...
extern uint8_t final_tx_right;
extern uint8_t final_ecu_id;
...
void UART_SERIAL_CALLBACK(uart_callback_args_t *p_args)
{
    ...
    if(Rx_SERIAL_DATA[2] == 'A' ) //CAN ID and MSG transmission
    {
        ...
        switch(p_args->data)
        {
            case'>':
                CAN_MSG_Tx();
                if(final_ecu_id == 16)APPW_SetText(ID_SCREEN_00, ID_TEXT_01, CAN_Tx_ID);
                else if(final_ecu_id != 16 && final_tx_right == 1) APPW_SetText(ID_SCREEN_00, ID_TEXT_01, CAN_Tx_ID);
                ...
                if(final_ecu_id == 16)APPW_SetText(ID_SCREEN_00, ID_TEXT_03, CAN_Buffer_t);
                else if(final_ecu_id != 16 && final_tx_right == 1)APPW_SetText(ID_SCREEN_00, ID_TEXT_03, CAN_Buffer_t);
                /* ---------------------------- */
...
void CAN_CALLBACK_A(can_callback_args_t *p_args)
{
    ...
    if(p_args->event == CAN_EVENT_RX_COMPLETE)
    {
        ...
        if(final_tx_right == 1)APPW_SetText(ID_SCREEN_00, ID_TEXT_00, CAN_RxID);
        /* ----------------------------- */
        ...
        if(final_tx_right == 1)APPW_SetText(ID_SCREEN_00, ID_TEXT_02, CAN_Buffer);
        /* ---------------------------- */
